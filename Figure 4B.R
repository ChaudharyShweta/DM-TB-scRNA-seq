...
seurat_data <- readRDS("integrated_d2d3.rds")
auc_1 = read.csv("auc_mtx.srat_d2.csv", header = FALSE)
auc_2 = read.csv("auc_mtx.srat_d3.csv", header = FALSE)
# set to false if looms were created using the non-integrated seurat object
IS_COUNT_DATA_EXTRACTED_FROM_INTEGRATED_RDS = FALSE
...
ha_1 <- HeatmapAnnotation(
  ...
  cell_name = plot_data_auc$customclassif[ident_1], show_annotation_name = FALSE,
  col = list(
    identifier = setNames(c("#009E73", "#0072B2"), nm = unique(plot_data_auc.binary$orig.ident)),
    cell_name = custom_colors_cellname
  )
)
...
ha_2 = HeatmapAnnotation(
  ...
  cell_name = plot_data_auc$customclassif[ident_2], show_annotation_name = TRUE,
  col = list(identifier = setNames(c("#009E73", "#0072B2"), nm = unique(plot_data_auc.binary$orig.ident)),
             cell_name = custom_colors_cellname
  )
)
